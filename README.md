...
- `telemetry.json` — telemetria (velocità, acceleratore, freno, marcia) del giro di pole in qualifica di ogni GP 2026 già disputato, da [OpenF1](https://openf1.org) (API pubblica diversa da f1api.dev, usata solo per questa funzione). Solo qualifiche (niente prove libere, sprint o gara) e solo la stagione 2026, su richiesta esplicita. Scaricato da `scripts/build-telemetry.js`, usato dal tasto "🏎️ Telemetria" del grafico principale.
...
- `scripts/build-telemetry.js` — strumento una tantum (`node scripts/build-telemetry.js`) che scarica da OpenF1 la telemetria del giro di pole in qualifica di ogni GP 2026 già disputato, e scrive `telemetry.json`. Trova il giro più veloce tra tutti quelli con un tempo registrato (nessun bisogno di sapere chi ha fatto la pole in anticipo); da rilanciare per aggiungere i round successivi mano a mano che si disputano (sovrascrive sempre il file intero, non incrementale come `build-history.js`).
