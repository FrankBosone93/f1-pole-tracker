...
- `calendar.json` — calendario della stagione IN CORSO (rilevata automaticamente, quindi passa da sola alla stagione successiva ogni anno), con il vincitore di ogni gara già disputata e la data di inizio weekend (`weekendStart`, prove libere del venerdì) di ognuna. Usato sia per il pannello "Calendario" (che mostra anche l'intervallo di date del weekend, es. "6-8 Marzo", calcolato da `weekendStart` e `date`) sia per decidere quale circuito mostrare di default all'apertura del sito (l'ultimo GP disputato, oppure quello del weekend in corso se le prove libere sono già iniziate).
